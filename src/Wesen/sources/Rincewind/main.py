...
class WesenSource(DefaultWesenSource):
    ...
    def continueOnCircle(self):
        ...
        radius = getShortestTranslation(
            self.midPoint, self.position(), self.infoAllSource["world"]["length"])
    ...
    def bestFoodInRange(self, foods):
        movingRange = int(
            (self.time() - self.infoAllSource["time"]["eat"]) / self.infoAllSource["time"]["move"])
        # TODO movingRange is not used after assignment!
        suitableFoods = [f for f in foods if f["age"] > 100]
        reachableFoods = list(filter(lambda f: getDistInMaxMetric(
            self.position(), f["position"], self.infoAllSource["world"]["length"]), suitableFoods))
    ...
    def protectFood(self):
        ...
        if not self.midPoint:
            ...
            self.midPoint = [int(c) % self.infoAllSource["world"]["length"]
